...
function bd09ToWgs84(lon, lat) {
  const [g1, g2] = bd09ToGcj02(lon, lat);
...
for (const dist of region.options || []) {
  ...
  for (const b of (dist.options || [])) {
    ...
    features.push({
      type: 'Feature',
      properties: { name: b.name, district },
...
console.log('贝壳板块边界解析完成，共', features.length, '个板块');

// 同时生成 boards.json（板块列表：name + district + center + count）
const boards = features.map(f => {
  ...
  return {
    ...
    location: (lon / n).toFixed(6) + ',' + (lat / n).toFixed(6),
    count: 10,
  };
});
fs.writeFileSync('data/boards.json', JSON.stringify(boards, null, 2));
console.log('boards.json 已更新，共', boards.length, '个板块');

const byDist = {};
features.forEach(f => { byDist[f.properties.district] = (byDist[f.properties.district] || 0) + 1; });
console.log('各区板块数:', JSON.stringify(byDist));
